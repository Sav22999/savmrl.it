...
if ($condition) {
    ...
    if ($c = new mysqli($localhost_db, $username_db, $password_db, $database_savmrl)) {
        ...
        //"c1" ("old name")
        //"c2" ("new name")
        $query_exists = "SELECT COUNT(*) AS `c1`, (SELECT COUNT(*) FROM `$redirect_table` WHERE `name` = ? AND `inserted_timestamp` >= (CURRENT_TIMESTAMP - INTERVAL 30 MINUTE)) AS `c2` FROM `$redirect_table` WHERE `name` = ? AND `inserted_timestamp` >= (CURRENT_TIMESTAMP - INTERVAL 30 MINUTE)";
